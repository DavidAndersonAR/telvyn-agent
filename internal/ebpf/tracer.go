...
type Tracer struct {
	...
	// lostSamples counts cumulative dropped samples per perf map name.
	// Bumped by runEventsReader whenever the kernel perf buffer overflowed
	// between Read() calls. Surfaced via LostSamples() for self-metrics
	// (agent_ebpf_lost_samples{map=...}) so operators can detect when the
	// ring buffers are undersized for their workload.
	lostSamplesMu sync.Mutex
	lostSamples   map[string]uint64
}
...
// LostSamples returns a copy of cumulative dropped samples per perf map
// name. Callers (selfmetrics) sample this periodically to emit the
// agent_ebpf_lost_samples{map=...} self-metric. Cumulative counters —
// VictoriaMetrics applies rate() in the backend dashboards.
func (t *Tracer) LostSamples() map[string]uint64 {
	t.lostSamplesMu.Lock()
	defer t.lostSamplesMu.Unlock()
	out := make(map[string]uint64, len(t.lostSamples))
	for k, v := range t.lostSamples {
		out[k] = v
	}
	return out
}
...
func (t *Tracer) ebpf(ch chan<- Event) error {
	...
	// Ring buffer sizing (per-CPU, in 4KB pages). Pages × CPUs × 4KB is the
	// kernel-side memory budget per perf map. Sized for the workload each
	// map sees: l7_events is the busiest (every HTTP/DB request → 1 event),
	// tcp_connect_events ranks next under churn (many short conns).
	//
	// L7=64 pages (256KB/CPU): bumped from 32 after observing LostSamples
	// spikes under pgbench-style sustained load (~1k qps). 256KB ≈ 2k events
	// per CPU at 128B avg event size, which absorbs the slowest userland
	// drain (gRPC backpressure) without dropping. Override via env
	// ISPWATCH_EBPF_L7_BUFFER_PAGES if needed.
	l7Pages := 64
	if v := os.Getenv("ISPWATCH_EBPF_L7_BUFFER_PAGES"); v != "" {
	...
	if !t.disableL7Tracing {
		perfMaps = append(perfMaps, perfMap{name: "l7_events", typ: perfMapTypeL7Events, perCPUBufferSizePages: l7Pages})
	}
...
func runEventsReader(name string, r *perf.Reader, ch chan<- Event, typ perfMapType, readTimeout time.Duration, t *Tracer) {
	if readTimeout == 0 {
	...
	for {
		...
		if rec.LostSamples > 0 {
			...
			if t != nil {
				t.lostSamplesMu.Lock()
				t.lostSamples[name] += rec.LostSamples
				t.lostSamplesMu.Unlock()
			}
			continue
		}
		var event Event
